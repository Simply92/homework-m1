...
- cacheFunction debe retornar una función. Esta función debe aceptar un argumento (arg) e invocar a cb con ese argumento; hecho eso, debe guardar el argumento junto con el resultado de 
la invocación (tip: usá un objeto donde cada propiedad sea el argumento, y su valor el resultado de la correspondiente invocación a cb) de manera que, 
...
function cacheFunction(cb) {
  ...
  return function(arg){
    if (arg in cache){           //if(cache.hasOwnProperty(arg))
      return cache[arg] 
    }
    var CallBackresultado = cb(arg)     //cache[arg] = cb(arg)
    resultado = CallBackresultado       //return chache[arg]
    cache[arg] = CallBackresultado
    return resultado;
  }
}
//----------------------------------------
